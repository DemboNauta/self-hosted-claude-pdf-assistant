...
Last updated: 2026-09-26 (multi-user). See [`README.md`](README.md) for the reading order.
...
- `apps/server`: 72 unit/integration tests pass (`vitest`), 9 of them for
  ...
  (`test/users.test.ts`).
- `apps/web`: unit tests pass. The Playwright suite has 38 tests over desktop
  and mobile projects: 33 pass and 5 are skipped on mobile by design (pointer
  drag, drawing, keyboard shortcuts). In the cloud sandbox this needed
  `PLAYWRIGHT_CHROMIUM_EXECUTABLE=/opt/pw-browsers/chromium` and the
  `getOrInsertComputed` polyfill (see "Known limitations"). The library drag-and-drop test is flaky
  (see `DEVELOPMENT.md`).
